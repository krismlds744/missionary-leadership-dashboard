...
export default function Retention() {
  ...
  return <div className="space-y-6 pb-10">
    ...
    <section className="rounded-[26px] border border-white/10 bg-slate-950/50 p-4 shadow-[0_24px_70px_rgba(2,8,23,0.2)] backdrop-blur-xl sm:p-5"><div className="mb-4 flex items-center justify-between"><div><p className="text-[0.65rem] uppercase tracking-[0.2em] text-slate-500">Directory</p><h2 className="mt-1 text-xl font-semibold text-white">Recent converts</h2></div><span className="text-xs text-slate-500">{converts.length} people</span></div><div className="grid gap-2">{converts.map((convert) => <button key={convert.id} type="button" onClick={() => { setSelectedId(convert.id); setOpenCategory('Covenants') }} className="flex min-h-14 items-center gap-3 rounded-xl border border-white/7 bg-white/[0.025] px-3 py-2 text-left transition hover:border-white/20 hover:bg-white/[0.06]"><div className="flex w-[240px] shrink-0 items-center gap-3"><div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-white/10 text-[0.65rem] font-semibold text-slate-300">{initials(convert.name)}</div><div className="min-w-0"><p className="truncate text-sm font-medium text-white">{convert.name}</p><p className="truncate text-xs text-slate-500">{convert.ward} · {convert.age} years old</p></div></div><div className="grid w-[360px] shrink-0 grid-cols-4 gap-2.5">{categories.map((category) => <CompactMeter key={category} convert={convert} category={category} store={store} />)}</div></button>)}</div></section>
