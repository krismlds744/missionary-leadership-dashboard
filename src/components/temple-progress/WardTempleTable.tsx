...
import type { TempleStatus, WardTempleRow } from '../../types/pageData'

interface WardTempleTableProps {
  rows: WardTempleRow[]
}

type SortKey = 'ward' | 'templeReady' | 'recommendHolders' | 'interviewsScheduled' | 'endowments' | 'sealings' | 'readiness'

const statusStyles: Record<TempleStatus, string> = {
...
export default function WardTempleTable({ rows }: WardTempleTableProps) {
  ...
  return (
    <div className="overflow-hidden rounded-[30px] border border-white/10 bg-slate-950/60 shadow-[0_30px_80px_rgba(15,23,42,0.24)] backdrop-blur-xl">
      <div className="flex items-center justify-between gap-4 border-b border-white/10 px-5 py-4 sm:px-6">
        <div>
          <p className="text-[0.7rem] uppercase tracking-[0.28em] text-slate-400">Ward readiness</p>
          <h3 className="mt-2 text-2xl font-semibold text-white">Temple readiness</h3>
        </div>
        ...
        <table className="min-w-full border-separate border-spacing-0 text-left">
          <thead>
            <tr className="bg-white/5 text-slate-300">
              {[
                ['ward', 'Ward'],
                ['templeReady', 'Temple Ready'],
                ['recommendHolders', 'Recommend Holders'],
          ...
          <tbody>
            {sortedRows.map((row) => (
              <tr key={row.ward} className="border-t border-white/10 transition-colors hover:bg-white/[0.03]">
                ...
                <td className="px-5 py-4 text-sm text-slate-200 sm:px-6">{row.recommendHolders}</td>
                <td className="px-5 py-4 text-sm text-slate-200 sm:px-6">{row.interviewsScheduled}</td>
                <td className="px-5 py-4 text-sm text-slate-200 sm:px-6">{row.endowments}</td>
                <td className="px-5 py-4 text-sm text-slate-200 sm:px-6">{row.sealings}</td>
                <td className="px-5 py-4 text-sm text-slate-200 sm:px-6">
                  <div className="flex items-center gap-2">
                    <div className="h-2.5 w-20 overflow-hidden rounded-full bg-slate-800/80">
                      <div className="h-full rounded-full bg-gradient-to-r from-emerald-400 via-amber-400 to-rose-400" style={{ width: `${row.readiness}%` }} />
                    </div>
                    <span>{row.readiness}%</span>
                  </div>
                </td>
                <td className="px-5 py-4 sm:px-6">
                  <span className={`inline-flex rounded-full border px-2.5 py-1 text-[0.62rem] font-medium uppercase tracking-[0.18em] ${statusStyles[row.status]}`}>
                    {row.status}
                  </span>
                </td>
              </tr>
