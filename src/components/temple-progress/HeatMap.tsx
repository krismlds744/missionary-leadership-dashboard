...
export default function HeatMap({ rows }: HeatMapProps) {
  return (
    <div className="rounded-[30px] border border-white/10 bg-slate-950/60 p-5 shadow-[0_30px_80px_rgba(15,23,42,0.24)] backdrop-blur-xl sm:p-6">
      <div className="mb-6">
        <p className="text-[0.7rem] uppercase tracking-[0.28em] text-slate-400">Readiness map</p>
        <h3 className="mt-2 text-2xl font-semibold text-white">Temple readiness heatmap</h3>
      </div>
          ...
          {rows.map((row) => (
            <>
              ...
              {row.values.map((value) => (
                <div key={`${row.ward}-${value}`} className={`flex items-center justify-center rounded-xl px-2 py-3 text-sm font-medium ${getColor(value)}`}>
                  {value}
                </div>
              ))}
            </>
