...
import type { OrdinanceEvent } from '../../types/pageData'

interface OrdinanceTimelineProps {
  items: OrdinanceEvent[]
}

export default function OrdinanceTimeline({ items }: OrdinanceTimelineProps) {
  return (
    <div className="rounded-[30px] border border-white/10 bg-slate-950/60 p-5 shadow-[0_30px_80px_rgba(15,23,42,0.24)] backdrop-blur-xl sm:p-6">
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          <p className="text-[0.7rem] uppercase tracking-[0.28em] text-slate-400">Recent history</p>
          <h3 className="mt-2 text-2xl font-semibold text-white">Ordinance timeline</h3>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {items.map((item) => (
          <div key={`${item.title}-${item.date}`} className="relative rounded-[24px] border border-white/10 bg-white/[0.03] p-4 pl-12">
            <div className="absolute left-4 top-5 h-3 w-3 rounded-full border border-emerald-300 bg-emerald-400" />
